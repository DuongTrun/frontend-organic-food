...
const ProductCardLG: React.FC<ProductCardProps> = ({
  id,
  image,
  name,
  price,
    ...
    <div className="relative group bg-white rounded-sm shadow hover:shadow-lg transition-all duration-300 overflow-hidden ">
      <div className="relative w-full h-64 flex justify-center items-center ">
        <img
          src={image}
          alt={name}
          className="object-contain h-full p-4 scale-125 transition-transform duration-300 group-hover:scale-105 "
        />
        ...
        <div className="absolute inset-0 flex items-center justify-center gap-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10">
          ...
          <button className="bg-white text-[#5bbb46] p-3 rounded-full hover:scale-105 transition" onClick={onView}>
            <FaEye size={18} />
          </button>
        </div>
      </div>
